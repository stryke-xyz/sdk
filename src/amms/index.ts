import { RH_CHAIN_ID } from "../chains";
import { tokens } from "../tokens";
import { AMM } from "../types/types";

export const amms: Record<number, AMM[]> = {
  4663: [
    {
      chainId: RH_CHAIN_ID,
      inversePrice: false,
      address: "0x7A192E71564ec66eE0763e328a3Ac274942dE4e1",
      feeBps: 10000,
      handler: "0x67599652ec14f635837929b4c08b1adc10f83300",
      ammVerboseName: "Uniswap V3",
      ammShortName: "uniswapv3",
      tickSpacing: 200,
      token0: tokens[4663].PONS,
      token1: tokens[4663].USDG,
      principleSymbol: "PONS",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: RH_CHAIN_ID,
      inversePrice: true,
      address: "0x3319CaF98A5C6C947Cb7e6eC6D45d0a8CA090668",
      feeBps: 2500,
      handler: "0x02a81b0eB603D1E228C4F3E5eD756E8b6EE64f11",
      ammVerboseName: "UP V3",
      ammShortName: "up-v3",
      tickSpacing: 60,
      token0: tokens[4663].USDG,
      token1: tokens[4663].STONKBROKER,
      principleSymbol: "STONKBROKER",
      quoteSymbol: "USD",
      isMemePair: true,
    },
    {
      chainId: RH_CHAIN_ID,
      inversePrice: false,
      address: "0xb77e03DF4CAe1752aa1E2b52C46794026b46873E",
      feeBps: 10000,
      handler: "0x67599652ec14f635837929b4c08b1adc10f83300",
      ammVerboseName: "Uniswap V3",
      ammShortName: "uniswap-v3",
      tickSpacing: 200,
      token0: tokens[4663].QUOTRON,
      token1: tokens[4663].USDG,
      principleSymbol: "QUOTRON",
      quoteSymbol: "USD",
      isMemePair: true,
    },
    {
      chainId: RH_CHAIN_ID,
      inversePrice: false,
      address: "0xe547c18f46Db55AB788343bcC503F9CF0bd7d564",
      feeBps: 10000,
      handler: "0x67599652ec14f635837929b4c08b1adc10f83300",
      ammVerboseName: "Uniswap V3",
      ammShortName: "uniswap-v3",
      tickSpacing: 200,
      token0: tokens[4663].AI,
      token1: tokens[4663].USDG,
      principleSymbol: "AI",
      quoteSymbol: "USD",
      isMemePair: true,
    },
    {
      chainId: RH_CHAIN_ID,
      inversePrice: true,
      address: "0x6f95aC65c37Adf4a58774c972d05483862B6bea7",
      feeBps: 9000,
      handler: "0x02a81b0eB603D1E228C4F3E5eD756E8b6EE64f11",
      ammVerboseName: "UP V3",
      ammShortName: "up-v3",
      tickSpacing: 100,
      token0: tokens[4663].USDG,
      token1: tokens[4663].BONER,
      principleSymbol: "BONER",
      quoteSymbol: "USD",
      isMemePair: true,
    },
  ],
  146: [
    {
      chainId: 146,
      inversePrice: false,
      address: "0x324963c267C354c7660Ce8CA3F5f167E05649970",
      feeBps: 1500,
      handler: "0x2B4d36702E1fAA7FE25c29D8200d050Dc3e17f60",
      ammVerboseName: "Shadow Exchange",
      ammShortName: "shadow-exchange",
      tickSpacing: 50,
      token0: tokens[146].WSONIC,
      token1: tokens[146].USDC,
      principleSymbol: "S",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 146,
      inversePrice: true,
      address: "0x6Fb30F3FCB864D49cdff15061ed5c6ADFEE40B40",
      feeBps: 1255,
      handler: "0x2B4d36702E1fAA7FE25c29D8200d050Dc3e17f60",
      ammVerboseName: "Shadow Exchange",
      ammShortName: "shadow-exchange",
      tickSpacing: 100,
      token0: tokens[146].USDC,
      token1: tokens[146].WETH,
      principleSymbol: "ETH",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 146,
      inversePrice: false,
      address: "0x8BC2f9e725cbB07c338df4e77c82190119ddd823",
      feeBps: 1054,
      handler: "0x2B4d36702E1fAA7FE25c29D8200d050Dc3e17f60",
      ammVerboseName: "Shadow Exchange",
      ammShortName: "shadow-exchange",
      tickSpacing: 100,
      token0: tokens[146].WBTC,
      token1: tokens[146].USDC,
      principleSymbol: "BTC",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 146,
      inversePrice: false,
      address: "0xb1BC4B830FCbA2184B92e15b9133c41160518038",
      feeBps: 619,
      handler: "0x0",
      ammVerboseName: "Equalizer",
      ammShortName: "equalizer",
      tickSpacing: 8,
      token0: tokens[146].WSONIC,
      token1: tokens[146].USDC,
      principleSymbol: "S",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 146,
      inversePrice: true,
      address: "0xfe809A1D337Bdfc98B77A1067e3819f66d8AD23F",
      feeBps: 1337,
      handler: "0x0",
      ammVerboseName: "Equalizer",
      ammShortName: "equalizer",
      tickSpacing: 8,
      token0: tokens[146].WETH,
      token1: tokens[146].USDC,
      principleSymbol: "ETH",
      quoteSymbol: "USD",
      isMemePair: false,
    },
  ],
  8453: [
    {
      chainId: 8453,
      inversePrice: true,
      address: "0x4e962BB3889Bf030368F56810A9c96B83CB3E778",
      feeBps: 351,
      handler: "0x1ce3588f5585586882399bcd7e9161dbEAa9640b",
      ammVerboseName: "Aerodrome",
      ammShortName: "aerodrome",
      tickSpacing: 100,
      token0: tokens[8453].CBBTC,
      token1: tokens[8453].USDC,
      principleSymbol: "BTC",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 8453,
      inversePrice: false,
      address: "0xb2cc224c1c9feE385f8ad6a55b4d94E92359DC59",
      feeBps: 422,
      handler: "0x1ce3588f5585586882399bcd7e9161dbEAa9640b",
      ammVerboseName: "Aerodrome",
      ammShortName: "aerodrome",
      tickSpacing: 100,
      token0: tokens[8453].WETH,
      token1: tokens[8453].USDC,
      principleSymbol: "ETH",
      quoteSymbol: "USD",
      isMemePair: false,
    },
  ],
  42161: [
    {
      chainId: 42161,
      inversePrice: false,
      address: "0xb0f6cA40411360c03d41C5fFc5F179b8403CdcF8",
      feeBps: 500,
      handler: "0x6F73aFB6598d7a3881577f884f2E01574aEFC373",
      ammVerboseName: "Uniswap V3",
      ammShortName: "uniswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].ARB,
      token1: tokens[42161].USDC,
      principleSymbol: "ARB",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0xfa1cc0cae7779b214b1112322a2d1cf0b511c3bc",
      feeBps: 500,
      handler: "0x0",
      ammVerboseName: "Sushiswap V3",
      ammShortName: "sushiswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].ARB,
      token1: tokens[42161].USDC,
      principleSymbol: "ARB",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0x9ffca51d23ac7f7df82da414865ef1055e5afcc3",
      feeBps: 500,
      handler: "0x0",
      ammVerboseName: "Pancakeswap V3",
      ammShortName: "pancakeswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].ARB,
      token1: tokens[42161].USDC,
      principleSymbol: "ARB",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0xC6962004f452bE9203591991D15f6b388e09E8D0",
      feeBps: 500,
      handler: "0x6F73aFB6598d7a3881577f884f2E01574aEFC373",
      ammVerboseName: "Uniswap V3",
      ammShortName: "uniswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].WETH,
      token1: tokens[42161].USDC,
      principleSymbol: "ETH",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0xf3eb87c1f6020982173c908e7eb31aa66c1f0296",
      feeBps: 500,
      handler: "0x0",
      ammVerboseName: "Sushiswap V3",
      ammShortName: "sushiswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].WETH,
      token1: tokens[42161].USDC,
      principleSymbol: "ETH",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0xd9e2a1a61b6e61b275cec326465d417e52c1b95c",
      feeBps: 500,
      handler: "0x0",
      ammVerboseName: "PancakeSwap V3",
      ammShortName: "pancakeswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].WETH,
      token1: tokens[42161].USDC,
      principleSymbol: "ETH",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0x0E4831319A50228B9e450861297aB92dee15B44F",
      feeBps: 500,
      handler: "0x6F73aFB6598d7a3881577f884f2E01574aEFC373",
      ammVerboseName: "Uniswap V3",
      ammShortName: "uniswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].WBTC,
      token1: tokens[42161].USDC,
      principleSymbol: "BTC",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0x699f628a8a1de0f28cf9181c1f8ed848ebb0bbdf",
      feeBps: 500,
      handler: "0x0",
      ammVerboseName: "Sushiswap V3",
      ammShortName: "sushiswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].WBTC,
      token1: tokens[42161].USDC,
      principleSymbol: "BTC",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0x843ac8dc6d34aeb07a56812b8b36429ee46bdd07",
      feeBps: 500,
      handler: "0x0",
      ammVerboseName: "Pancakeswap V3",
      ammShortName: "pancakeswap-v3",
      tickSpacing: 10,
      token0: tokens[42161].WBTC,
      token1: tokens[42161].USDC,
      principleSymbol: "BTC",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 42161,
      inversePrice: false,
      address: "0xe24F62341D84D11078188d83cA3be118193D6389",
      feeBps: 10000,
      handler: "0x0",
      ammVerboseName: "Uniswap V3",
      ammShortName: "uniswap-v3",
      tickSpacing: 200,
      token0: tokens[42161].BOOP,
      token1: tokens[42161].WETH,
      principleSymbol: "BOOP",
      quoteSymbol: "ETH",
      isMemePair: true,
    },
  ],
  80094: [
    {
      chainId: 80094,
      inversePrice: false,
      address: "0x9EB897D400f245E151daFD4c81176397D7798C9c",
      feeBps: 3000,
      handler: "0xb0d4E5F539A82457d5D2C16Df17b00b5D6373Bf9",
      ammVerboseName: "Kodiak V3",
      ammShortName: "kodiak-v3",
      tickSpacing: 60,
      token0: tokens[80094].WETH,
      token1: tokens[80094].HONEY,
      principleSymbol: "ETH",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 80094,
      inversePrice: false,
      address: "0x545Bea6Ea7F8fD8dCC5C9A6802a8ebF3DbFc1C6E",
      feeBps: 3000,
      handler: "0xb0d4E5F539A82457d5D2C16Df17b00b5D6373Bf9",
      ammVerboseName: "Kodiak V3",
      ammShortName: "kodiak-v3",
      tickSpacing: 60,
      token0: tokens[80094].WBTC,
      token1: tokens[80094].HONEY,
      principleSymbol: "BTC",
      quoteSymbol: "USD",
      isMemePair: false,
    },
    {
      chainId: 80094,
      inversePrice: false,
      address: "0x1127f801Cb3ab7BDF8923272949AA7Dba94B5805",
      feeBps: 3000,
      handler: "0xb0d4E5F539A82457d5D2C16Df17b00b5D6373Bf9",
      ammVerboseName: "Kodiak V3",
      ammShortName: "kodiak-v3",
      tickSpacing: 60,
      token0: tokens[80094].WBERA,
      token1: tokens[80094].HONEY,
      principleSymbol: "BERA",
      quoteSymbol: "USD",
      isMemePair: false,
    },
  ],
  1439: [
    {
      chainId: 1439,
      inversePrice: false,
      address: "0x22A0C8035d67b01390A93E004E615e62FB147372",
      feeBps: 500,
      handler: "0xeDad433bF77E409c05215D4679e4b7de20C9A8Dd",
      ammVerboseName: "Borderless",
      ammShortName: "borderless",
      tickSpacing: 10,
      token0: tokens[1439].WINJ,
      token1: tokens[1439].USDC,
      principleSymbol: "INJ",
      quoteSymbol: "USD",
      isMemePair: false,
    },
  ],
};
